package mitrais.bootcamp.scrummit.multitenancy;
...
public class MultiTenantMongoDbFactory extends SimpleMongoDbFactory {
    ...
    private static final Log                     logger           = LogFactory.getLog(MultiTenantMongoDbFactory.class);
    ...
    public MultiTenantMongoDbFactory(final MongoClient mongo, final String defaultDatabaseName) {
        ...
        super.getDb(defaultDatabaseName);
    }
    ...
    private void createIndexIfNecessaryFor(final String database) {
        if (this.mongoTemplate == null) {
            logger.warn("MongoTemplate is null, will not create any index.");
            return;
        }
    ...
    private void checkForAndCreateIndexes(final MongoPersistentEntityIndexResolver indexResolver, final MongoPersistentEntity<?> entity) {
        ...
        if (entity.findAnnotation(Document.class) != null) {
            for (IndexDefinitionHolder indexDefinitionHolder : indexResolver.resolveIndexFor(entity
                    .getTypeInformation())) {
